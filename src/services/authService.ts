...
class AuthService {
  ...
  async login(email: string, password: string): Promise<LoginResponse> {
    try {
      console.log('AuthService: Making login request to:', `${this.API_BASE_URL}/Account/Login`);
      console.log('AuthService: Environment:', { isDev: this.isDevelopment(), apiUrl: this.API_BASE_URL });
      console.log('AuthService: Request payload:', { login: email, password: '***' });
      ...
      this.setTokens(data.value.accessToken, data.value.refreshToken);
      this.setUser(user);
  ...
  async refreshToken(): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(`${this.API_BASE_URL}/Account/RefreshToken`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
      ...
      if (data?.value?.accessToken) {
        this.setTokens(data.value.accessToken, data.value.refreshToken || refreshToken);
        return true;
      }
